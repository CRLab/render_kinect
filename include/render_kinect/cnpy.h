...
namespace cnpy {
    ...
    template<typename T>
    std::vector<char> &operator+=(std::vector<char> &lhs, T rhs) {
    ...
    template<typename T>
    void
    npy_save(const std::string &fname, const T *data, const std::vector<size_t> &shape, const std::string &mode = "w") {
        FILE *fp = nullptr;
        ...
        size_t nels = static_cast<size_t>(std::accumulate(shape.begin(), shape.end(), 1, std::multiplies<size_t>()));
    ...
    template<typename T>
    void npz_save(const std::string &zipname, std::string fname, const T *data, const std::vector<size_t> &shape,
        ...
        auto crc = static_cast<uint32_t>(crc32(0L, (uint8_t *) &npy_header[0], static_cast<uInt>(npy_header.size())));
        crc = static_cast<uint32_t>(crc32(crc, (uint8_t *) data, static_cast<uInt>(nels * sizeof(T))));
    ...
    std::vector<char> create_npy_header(const std::vector<size_t> &shape) {
        ...
        int remainder = static_cast<int>(16 - (10 + dict.size()) % 16);
        dict.insert(dict.end(), static_cast<unsigned long>(remainder), ' ');
        dict.back() = '\n';
